...
/**
 * @param {ObservationMeasurement} observation
 * @return {import('influx').IPoint}
 */
function observationToPoint(observation) {
  return {
    measurement: `dmi.${observation.parameterId}`,
    fields: {
  ...
  try {
    ...
    async function iterate() {
      ...
      try {
        ...
        const points = observations.features.map(o => o.properties).map(observationToPoint)
        if (points.length > 0) {
